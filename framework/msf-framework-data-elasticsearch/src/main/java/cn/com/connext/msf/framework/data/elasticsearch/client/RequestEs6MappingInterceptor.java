 ...
 * 用于兼容ES6.X中index的type类型
 */
public class RequestEs6MappingInterceptor implements ElasticSearchRequestInterceptor {
    private final Logger logger = LoggerFactory.getLogger(RequestEs6MappingInterceptor.class);
    ...
    public void process(HttpRequest request, HttpContext context) throws HttpException, IOException {
        ...
        switch (requestMethod) {
            ...
            case "POST":
                if (uriBuilder.getPath().indexOf("_update") > -1) {
                    rebuildDataUpdateUri(requestWrapper, uriBuilder);
                } else if (uriBuilder.getPath().endsWith("_bulk")) {
    ...
    private void rebuildMappingUri(HttpRequestWrapper requestWrapper, URIBuilder uriBuilder) {
        uriBuilder.setPath(uriBuilder.getPath() + "/_doc");
     ...
     * @param uriBuilder
     */
    private void rebuildDataUpdateUri(HttpRequestWrapper requestWrapper, URIBuilder uriBuilder) {
        uriBuilder.setPath(uriBuilder.getPath().replace("_update", "_doc") + "/_update");
        RequestParamsInterceptor.rebuildRequestUri(requestWrapper, uriBuilder, logger);
    }
    ...
    private void rebuildBulkUri(HttpRequestWrapper requestWrapper, URIBuilder uriBuilder) throws IOException {
        ...
        if (contentList.length > 0) {
            ...
            if (StringUtils.isNotEmpty(indexName)) {
                uriBuilder.setPath("/" + indexName + "/_doc" + uriBuilder.getPath());
                RequestParamsInterceptor.rebuildRequestUri(requestWrapper, uriBuilder, logger);
            }
        }
    }
